/* projects.js — Page-specific logic only. Shared utilities in utils.js */
...
function createProjectCard(project) {
  ...
  const stackHTML = project.stack.map(t => `<span class="stack-tag">${t}</span>`).join('');
  ...
  return `
    <div class="project-card">
      <div class="card-image">${imageSection}</div>
      <div class="card-body">
        <div class="card-title">${project.title}</div>
        <div class="card-desc">${project.description}</div>
        <div class="card-stack">${stackHTML}</div>
      </div>
      <div class="card-footer">${githubBtn}${demoBtn}</div>
    </div>`;
}

function renderProjects() {
  document.getElementById('projectsGrid').innerHTML = projectsData.map(createProjectCard).join('');
}
